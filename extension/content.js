function addElements() {
  ...
  const joinLink = document.querySelector('a.nav-link[href="https://join-lemmy.org"]');
  const createPostLink = document.querySelector('a.btn.btn-secondary.btn-block.mb-2[href^="/create_post?community_id="]');
  ...
  if (loginLink && joinLink && createPostLink && !existingSubscribeLink) {
    ...
    if (!regexResult) {
      console.error('Unable to extract current server and community name from URL.');
      return;
    }
